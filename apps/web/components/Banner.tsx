...
const defaultIcons: Record<BannerVariant, React.ReactElement> = {
  success: (
...
export const BannerProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const push = useCallback<BannerContextType["push"]>(
...
export const BannerHost: React.FC<{ className?: string }> = ({ className }) => {
  ...
  if (!ctx)
    throw new Error("<BannerHost /> must be used under <BannerProvider />.");

  return (
    <div
      className={`fixed top-20 left-1/2 -translate-x-1/2 z-40 w-full max-w-2xl px-4 space-y-2 ${className ?? ""}`}
    >
      {ctx.items.map(
        ({ id, variant, message, autoHideMs, icon, className: itemClass }) => {
          ...
          return (
            <div
              key={id}
              className={`mb-4 rounded-md border ${border} ${bg} p-4 ${itemClass ?? ""}`}
              role={role}
              aria-live={ariaLive}
            >
              <div className="flex">
                ...
                <div
                  className={`ml-3 flex-1 ${icon === false ? "" : "mt-0.5"}`}
                >
...
export const useBanner = () => {
  ...
  const success = (message: React.ReactNode, opts?: PushOptions) =>
    ctx.push("success", message, opts);
  const error = (message: React.ReactNode, opts?: PushOptions) =>
    ctx.push("error", message, opts);
  const info = (message: React.ReactNode, opts?: PushOptions) =>
    ctx.push("info", message, opts);
  const warning = (message: React.ReactNode, opts?: PushOptions) =>
    ctx.push("warning", message, opts);
