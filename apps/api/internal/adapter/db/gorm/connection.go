// apps/api/internal/adapter/db/gorm/connection.go
// データベース接続を管理するパッケージ
// GORM（Go ORM）を使用してMySQL 8.0への接続とコネクションプールの設定を行う
package gorm
...
func NewDB(cfg configs.DatabaseConfig) (*gorm.DB, error) {
	...
	dsn := fmt.Sprintf(
		"%s:%s@tcp(%s:%s)/%s?parseTime=true&loc=Local&charset=utf8mb4&collation=utf8mb4_0900_ai_ci",
		cfg.User,
		cfg.Password,
		cfg.Host,
		cfg.Port,
