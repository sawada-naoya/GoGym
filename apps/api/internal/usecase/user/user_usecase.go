...
func (uc *UseCase) Signup(ctx context.Context, req SignupRequest) (*AuthResponse, error) {
	...
	// Create user entity
	newUser, err := user.NewUser(req.Email, req.DisplayName)
	if err != nil {
	...
	if err := uc.userRepo.Create(ctx, newUser); err != nil {
		...
		return nil, common.NewDomainError(err, "user_creation_failed", "failed to create user")
	}

	// Generate tokens
	accessToken, refreshToken, err := uc.tokenService.GenerateTokens(newUser.ID, newUser.Email)
	if err != nil {
...
func (uc *UseCase) Login(ctx context.Context, req LoginRequest) (*AuthResponse, error) {
	...
	// Generate new tokens
	accessToken, refreshToken, err := uc.tokenService.GenerateTokens(foundUser.ID, foundUser.Email)
	if err != nil {
...
func (uc *UseCase) RefreshToken(ctx context.Context, refreshTokenStr string) (*AuthResponse, error) {
	...
	// Generate new tokens
	newAccessToken, newRefreshToken, err := uc.tokenService.GenerateTokens(foundUser.ID, foundUser.Email)
	if err != nil {
...
func (uc *UseCase) saveRefreshToken(ctx context.Context, userID common.ID, refreshToken string) error {
	...
	if err := uc.refreshTokenRepo.Create(ctx, refreshTokenEntity); err != nil {
		return common.NewDomainError(err, "token_save_failed", "failed to save refresh token")
	}
